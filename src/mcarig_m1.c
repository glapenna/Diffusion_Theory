#include "head.h"

// extern char		tok[];
// char		tok[];
// void *get2(int,int);

/**** extern ****/
extern idstruct		*startid(),*getstruct();

rotstruct **rot_matrix();
_REAL S_switch[1000];
void setup_switch(void);

/************************************************************************/

void
init_mcarig()
{
	idstruct	*dptr;
	char		id[TOKLEN];
	int		exists;

	dptr = startid(MCARIG, id);

	dptr->idunion.mcarig.cases = 0;
	dptr->idunion.mcarig.casesw = 0.;
	dptr->idunion.mcarig.dump = 0;
	dptr->idunion.mcarig.rigid = 0;
	dptr->idunion.mcarig.sel_modes = 3;
	dptr->idunion.mcarig.list = 0;
	dptr->idunion.mcarig.tens = 0;
	dptr->idunion.mcarig.n2 = 0;

	/* 'file_name' */

	gettok();
	if (iskeywd())
		inerr("MCARIG: expected file name","");
	strcpy(dptr->idunion.mcarig.fname, tok);

	/*
	 *  open the file(s)
	 */

	if (fileprob(dptr->idunion.mcarig.fname, 0, &exists, "MCARIG"))
		exit(1);
	if ((dptr->idunion.mcarig.file = 
				  fopen(dptr->idunion.mcarig.fname, "w")) == NULL){
		perror(dptr->idunion.mcarig.fname);
		exit(1);
	}
	gettok();

/*
printf("dptr %d id %s\n", dptr, dptr->id_name);
*/
}

/************************************************************************/

void
setup_mcarig_m1()
{
	idstruct	*dptr,*bptr;
	int i,j,k,offset_r,nt,m1,m2;
        FILE *ftmp;

/* get id */
        gettok();
        if ((dptr = getstruct(tok)) != NULL) {
                if (dptr->id_type != MCARIG)
                        inerr("setup_mcarig_m1: expected id or keyword\n","");
        } else {
                printf("setup_mcarig_m1: id %s not found in idlist\n", tok);
        }
        gettok();
        if ((bptr = getstruct(tok)) == NULL) 
                inerr("setup_mcarig_m1: no such id\n","");
        if (bptr->id_type != BEAD)
                inerr("setup_mcarig_m1: bead id expected\n","");

        dptr->idunion.mcarig.type = 0;
        dptr->idunion.mcarig.sel_bonds = 0;

        gettok();
        for(;;) {
         if(tok[0]==';') {
          break;
         } else if(!strcmp(tok,"READFSI")) {
          dptr->idunion.mcarig.dump=-1;
          gettok();
         } else if(!strcmp(tok,"WRITEFSI")) {
          dptr->idunion.mcarig.dump=1;
          gettok();
         } else if(!strcmp(tok,"LOCAL")) {
          dptr->idunion.mcarig.type=1;
          gettok();
         } else if(!strcmp(tok,"RIGID")) {
          dptr->idunion.mcarig.rigid=1;
          gettok();
         } else if(!strcmp(tok,"NMODES")) {
          gettok();
          tokint(&dptr->idunion.mcarig.sel_modes,tok);
          dptr->idunion.mcarig.type=2;
          gettok();
         } else if(!strcmp(tok,"NBONDS")) {
          gettok();
          tokint(&dptr->idunion.mcarig.sel_bonds,tok);
          gettok();
         } else if(!strcmp(tok,"LIST")) {
          dptr->idunion.mcarig.list=1;
          gettok();
         } else if(!strcmp(tok,"TENS")) {
          dptr->idunion.mcarig.tens=1;
          gettok();
         } else {
          printf("setup_mcarig_m1: option not found.\n");
          exit(1);
         }
        }

        dptr->idunion.mcarig.bead = bptr;
       dptr->idunion.mcarig.n1=bptr->idunion.bead.nbond;
       nt=0;

       if(dptr->idunion.mcarig.sel_bonds!=0)
       {

        if(dptr->idunion.mcarig.list==0)
        {

        bptr->idunion.bead.l->offset_l = (int *)
          get2(dptr->idunion.mcarig.n1,sizeof(int));
        bptr->idunion.bead.l->doffset = (int *)
          get2(dptr->idunion.mcarig.n1,sizeof(int));

        nt=0;
        for(i=0;i<bptr->idunion.bead.nbond;i++) {
         bptr->idunion.bead.l->offset_l[i]= dptr->idunion.mcarig.sel_bonds -1;
                            offset_r   = dptr->idunion.mcarig.sel_bonds -1;
         bptr->idunion.bead.l->doffset[i]=  offset_r
                                        +bptr->idunion.bead.l->offset_l[i] +1;

         if(i-bptr->idunion.bead.l->offset_l[i] < 0)
         {
          bptr->idunion.bead.l->offset_l[i]-=dptr->idunion.mcarig.sel_bonds-i-1;
         }
         if(i+offset_r > bptr->idunion.bead.nbond-1)
         {
                            offset_r   -=i+1
                                        +dptr->idunion.mcarig.sel_bonds-1
                                        -bptr->idunion.bead.nbond;
         }
         bptr->idunion.bead.l->doffset[i]=  offset_r
                                        +bptr->idunion.bead.l->offset_l[i] +1;
/*
         printf("offset(%d)= %d %d\n",
          i,bptr->idunion.bead.l->offset_l[i],bptr->idunion.bead.l->doffset[i]);
         fflush(stdout);
*/

         nt+=bptr->idunion.bead.l->doffset[i]*
             (bptr->idunion.bead.l->doffset[i]+1)/2;

         }

        }
        else
        {
          ftmp=fopen("j1j2j3.selected","r");
          nt=0;
          while( !feof(ftmp) ) 
          {
           fscanf(ftmp,"%*s %*d %*d %*d %*d ");
           nt++;
          }
          printf("setup_mcarig_m1: reading %d entries from file j1j2j3.selected\n",nt);
          fflush(stdout);
          dptr->idunion.mcarig.nlist=nt;
          dptr->idunion.mcarig.j1=(int *)get2(nt,sizeof(int));
          dptr->idunion.mcarig.j2=(int *)get2(nt,sizeof(int));
          dptr->idunion.mcarig.j3=(int *)get2(nt,sizeof(int));
          rewind(ftmp);
          nt=0;
          while( !feof(ftmp) ) 
          {
           fscanf(ftmp,"%*s %*d %d %d %d ",
              &dptr->idunion.mcarig.j1[nt],
              &dptr->idunion.mcarig.j2[nt],
              &dptr->idunion.mcarig.j3[nt]);
           dptr->idunion.mcarig.j1[nt]--;
           dptr->idunion.mcarig.j2[nt]--;
           dptr->idunion.mcarig.j3[nt]--;
           nt++;
          }
          if(nt!=dptr->idunion.mcarig.nlist)
          {
           printf("setup_mcarig_m1: error reading file j1j2j3.selected.\n");
           exit(1);
          }
          fclose(ftmp);

        }
       }

       if(dptr->idunion.mcarig.sel_bonds!=0)
       {
         dptr->idunion.mcarig.l123 = (mod123struct *)
           get2(nt,sizeof(mod123struct));
       }

       dptr->idunion.mcarig.n2=nt;

       dptr->idunion.mcarig.dim1=
        dptr->idunion.mcarig.n1*(dptr->idunion.mcarig.n1+1)/2;
       if(dptr->idunion.mcarig.tens==0)
       {
        dptr->idunion.mcarig.s1 = (_REAL *)
          get2(dptr->idunion.mcarig.dim1,sizeof(_REAL));
        dptr->idunion.mcarig.f1 = (_REAL *)
          get2(dptr->idunion.mcarig.dim1,sizeof(_REAL));
        for(i=0;i<dptr->idunion.mcarig.dim1;i++) {
         *dptr->idunion.mcarig.s1++ =0.;
         *dptr->idunion.mcarig.f1++ =0.;
        }
        dptr->idunion.mcarig.s1-=dptr->idunion.mcarig.dim1;
        dptr->idunion.mcarig.f1-=dptr->idunion.mcarig.dim1;
       }
       else
       {
        dptr->idunion.mcarig.st1 = (rotstruct *)
          get2(dptr->idunion.mcarig.dim1,sizeof(rotstruct));
        dptr->idunion.mcarig.ft1 = (rotstruct *)
          get2(dptr->idunion.mcarig.dim1,sizeof(rotstruct));
        for(i=0;i<dptr->idunion.mcarig.dim1;i++) {
         for(m1=0;m1<3;m1++) 
         {
          for(m2=0;m2<3;m2++) 
          {
           dptr->idunion.mcarig.st1->f[m1][m2] =0.;
           dptr->idunion.mcarig.ft1->f[m1][m2] =0.;
          }
         }
         dptr->idunion.mcarig.st1++;
         dptr->idunion.mcarig.ft1++;
        }
        dptr->idunion.mcarig.st1-=dptr->idunion.mcarig.dim1;
        dptr->idunion.mcarig.ft1-=dptr->idunion.mcarig.dim1;

       }


       if(dptr->idunion.mcarig.sel_bonds!=0)
       {

        dptr->idunion.mcarig.dim12=
         dptr->idunion.mcarig.n1*dptr->idunion.mcarig.n2;
        dptr->idunion.mcarig.dim2=
         dptr->idunion.mcarig.n2*(dptr->idunion.mcarig.n2+1)/2;

        if(dptr->idunion.mcarig.tens==0)
        {
         dptr->idunion.mcarig.s12 = (_REAL *)
          get2(dptr->idunion.mcarig.dim12,sizeof(_REAL));
         dptr->idunion.mcarig.f12 = (_REAL *)
          get2(dptr->idunion.mcarig.dim12,sizeof(_REAL));
         for(i=0;i<dptr->idunion.mcarig.dim12;i++) {
          *dptr->idunion.mcarig.s12++ =0.;
          *dptr->idunion.mcarig.f12++ =0.;
         }
         dptr->idunion.mcarig.s2 = (_REAL *)
          get2(dptr->idunion.mcarig.dim2,sizeof(_REAL));
         dptr->idunion.mcarig.f2 = (_REAL *)
          get2(dptr->idunion.mcarig.dim2,sizeof(_REAL));
         for(i=0;i<dptr->idunion.mcarig.dim2;i++) {
          *dptr->idunion.mcarig.s2++ =0.;
          *dptr->idunion.mcarig.f2++ =0.;
         }
         dptr->idunion.mcarig.s12-=dptr->idunion.mcarig.dim12;
         dptr->idunion.mcarig.f12-=dptr->idunion.mcarig.dim12;
         dptr->idunion.mcarig.s2-=dptr->idunion.mcarig.dim2;
         dptr->idunion.mcarig.f2-=dptr->idunion.mcarig.dim2;
        }
        else
        {

         dptr->idunion.mcarig.st12 = (rotstruct *)
          get2(dptr->idunion.mcarig.dim12,sizeof(rotstruct));
         dptr->idunion.mcarig.ft12 = (rotstruct *)
          get2(dptr->idunion.mcarig.dim12,sizeof(rotstruct));
         for(i=0;i<dptr->idunion.mcarig.dim12;i++) {
          for(m1=0;m1<3;m1++) 
          {
           for(m2=0;m2<3;m2++) 
           {
            dptr->idunion.mcarig.st12->f[m1][m2] =0.;
            dptr->idunion.mcarig.ft12->f[m1][m2] =0.;
           }
          }
          dptr->idunion.mcarig.st12++;
          dptr->idunion.mcarig.ft12++;
         }
         dptr->idunion.mcarig.st2 = (rotstruct *)
          get2(dptr->idunion.mcarig.dim2,sizeof(rotstruct));
         dptr->idunion.mcarig.ft2 = (rotstruct *)
          get2(dptr->idunion.mcarig.dim2,sizeof(rotstruct));
         for(i=0;i<dptr->idunion.mcarig.dim2;i++) {
          for(m1=0;m1<3;m1++) 
          {
           for(m2=0;m2<3;m2++) 
           {
            dptr->idunion.mcarig.st2->f[m1][m2] =0.;
            dptr->idunion.mcarig.ft2->f[m1][m2] =0.;
           }
          }
          dptr->idunion.mcarig.st2++;
          dptr->idunion.mcarig.ft2++;
         }
         dptr->idunion.mcarig.st12-=dptr->idunion.mcarig.dim12;
         dptr->idunion.mcarig.ft12-=dptr->idunion.mcarig.dim12;
         dptr->idunion.mcarig.st2-=dptr->idunion.mcarig.dim2;
         dptr->idunion.mcarig.ft2-=dptr->idunion.mcarig.dim2;
        }


       }

        dptr->idunion.mcarig.nn=dptr->idunion.mcarig.n1+dptr->idunion.mcarig.n2;

        printf("setup_mcarig: n1:    %d\n",dptr->idunion.mcarig.n1);
        printf("setup_mcarig: n2:    %d\n",dptr->idunion.mcarig.n2);
        printf("setup_mcarig: nn:    %d\n",dptr->idunion.mcarig.nn);
        printf("setup_mcarig: dim1:  %d\n",dptr->idunion.mcarig.dim1);
        printf("setup_mcarig: dim12: %d\n",dptr->idunion.mcarig.dim12);
        printf("setup_mcarig: dim2:  %d\n",dptr->idunion.mcarig.dim2);

/*
        dptr->idunion.mcarig.Rm1 = matrix(
              1,bptr->idunion.bead.nbead,
              1,bptr->idunion.bead.nbead);
        for(i=1;i<=bptr->idunion.bead.nbead;i++) {
         for(j=1;j<=bptr->idunion.bead.nbead;j++) {
          dptr->idunion.mcarig.Rm1[i][j] =0.;
         }
        }
*/
        dptr->idunion.mcarig.Fb = rot_matrix
         (0,bptr->idunion.bead.nbond-1,0,bptr->idunion.bead.nbond-1);

        /* setup_switch(); */

}

/************************************************************************/

do_first_m1(idstruct *objptr,idstruct *serptr)
{
   _REAL *box=serptr->idunion.ser.set->boxc;
   idstruct *bptr;
   int nbond, *offset_l, *doffset;
   int i,j,kk,l,i1,i2,i3,j1,j2,j3,j1_0,j2_0,j3_0,ib,jb,k,m1,m2,overlap,
       i1_0,i2_0,i3_0,i1_1,i2_1,i3_1,j1_1,j2_1,j3_1,ii,jj,jj0;
   _REAL **r1, **r2;
   _REAL *rt1,*rt2,zi,zj,zr,cutsq;
   _REAL boxm1[3],x1,y1,z1,x2,y2,z2,dx,dy,dz,scp;
   bondstruct *lb1, *lb2, *lb,*p1,*p2,*p3;
   mod123struct *l123,*l123_1,*l123_2;
   _REAL sig0r,f[3][3],weight;
   extern _REAL S_switch[1000];

   sig0r=serptr->idunion.ser.prm->sig0r;
   bptr=objptr->idunion.mcarig.bead;
   zr=bptr->idunion.bead.zr;
   cutsq=bptr->idunion.bead.cutsq;
   nbond=bptr->idunion.bead.nbond;

   if(objptr->idunion.mcarig.sel_bonds!=0)
   {
    offset_l=bptr->idunion.bead.l->offset_l;
    doffset=bptr->idunion.bead.l->doffset;
   }

   for(k=0;k<3;k++) {
    boxm1[k]=1./box[k];
   }

   if(serptr->idunion.ser.iweight)
   {
    weight=serptr->idunion.ser.w->idunion.serw.w;
   } else {
    weight=1.;
   }

   ++objptr->idunion.mcarig.cases;
   objptr->idunion.mcarig.casesw+=weight;

/*
   r1=bptr->idunion.bead.r;
   r2=bptr->idunion.bead.r;
   for (i=1;i<=bptr->idunion.bead.nbead-1;i++,r1++) {
    rt1=*r1;;
    x1=rt1[0]; y1=rt1[1]; z1=rt1[2];
    for (j=i+1,r2=r1+1;j<=bptr->idunion.bead.nbead;j++,r2++) {
     rt2=*r2;
     x2=rt2[0]; y2=rt2[1]; z2=rt2[2];
     dx=x1-x2;
     dy=y1-y2;
     dz=z1-z2;
     scp = dx*dx+dy*dy+dz*dz;
     scp = 1./l_sqrt(scp);
     objptr->idunion.mcarig.Rm1[i][j]+=scp;
    }
   }
   return;
*/

   lb=bptr->idunion.bead.l;
   r1=bptr->idunion.bead.r1;
   r2=bptr->idunion.bead.r2;
   for (i=0;i<bptr->idunion.bead.nbond;i++,r1++,r2++) {
    rt1=*r1; rt2=*r2;
    x1=rt1[0]; y1=rt1[1]; z1=rt1[2];
    x2=rt2[0]; y2=rt2[1]; z2=rt2[2];
/*
   printf("do_first_m1:r1(%d): %e %e %e\n",i+1,sig0r*x1,sig0r*y1,sig0r*z1);
   printf("do_first_m1:r2(%d): %e %e %e\n",i+1,sig0r*x2,sig0r*y2,sig0r*z2);
   fflush(stdout);
*/

    dx=x1-x2;
    dy=y1-y2;
    dz=z1-z2;
    lb[i].id1 = (int)(*r1);
    lb[i].id2 = (int)(*r2);
    lb[i].x1 = x1;
    lb[i].x2 = x2;
    lb[i].y1 = y1;
    lb[i].y2 = y2;
    lb[i].z1 = z1;
    lb[i].z2 = z2;
#ifdef PBC
    dx -= box[0]*rint(dx*boxm1[0]);
    dy -= box[1]*rint(dy*boxm1[1]);
    dz -= box[2]*rint(dz*boxm1[2]);
#endif
    lb[i].x = dx;
    lb[i].y = dy;
    lb[i].z = dz;
    lb[i].sq = dx*dx+dy*dy+dz*dz;
    bptr->idunion.bead.bavt += lb->sq;
    bptr->idunion.bead.bavt2 += lb->sq*lb->sq;
   }
   r1-=bptr->idunion.bead.nbond;
   r2-=bptr->idunion.bead.nbond;

   if(bptr->idunion.bead.bav==0) return;

   if(objptr->idunion.mcarig.sel_bonds!=0)
   {

   l123=objptr->idunion.mcarig.l123;
   lb=bptr->idunion.bead.l;
   for (i1=0,p1=lb;i1<nbond;i1++,p1++) {
    for (i2=0,p2=p1-offset_l[i1];i2<doffset[i1];i2++,p2++) {
     for (i3=i2,p3=p2;i3<doffset[i1];i3++,p3++) {
       scp=p2->x*p3->x+p2->y*p3->y+p2->z*p3->z;
       l123->phi[0]=p1->x*scp;
       l123->phi[1]=p1->y*scp;
       l123->phi[2]=p1->z*scp;
       l123->dphidm1[0][0]=rdelta(0,0)*scp;
       l123->dphidm2[0][0]=p1->x*p3->x;
       l123->dphidm3[0][0]=p1->x*p2->x;
       l123->dphidm1[0][1]=rdelta(0,1)*scp;
       l123->dphidm2[0][1]=p1->x*p3->y;
       l123->dphidm3[0][1]=p1->x*p2->y;
       l123->dphidm1[0][2]=rdelta(0,2)*scp;
       l123->dphidm2[0][2]=p1->x*p3->z;
       l123->dphidm3[0][2]=p1->x*p2->z;
       l123->dphidm1[1][0]=rdelta(1,0)*scp;
       l123->dphidm2[1][0]=p1->y*p3->x;
       l123->dphidm3[1][0]=p1->y*p2->x;
       l123->dphidm1[1][1]=rdelta(1,1)*scp;
       l123->dphidm2[1][1]=p1->y*p3->y;
       l123->dphidm3[1][1]=p1->y*p2->y;
       l123->dphidm1[1][2]=rdelta(1,2)*scp;
       l123->dphidm2[1][2]=p1->y*p3->z;
       l123->dphidm3[1][2]=p1->y*p2->z;
       l123->dphidm1[2][0]=rdelta(2,0)*scp;
       l123->dphidm2[2][0]=p1->z*p3->x;
       l123->dphidm3[2][0]=p1->z*p2->x;
       l123->dphidm1[2][1]=rdelta(2,1)*scp;
       l123->dphidm2[2][1]=p1->z*p3->y;
       l123->dphidm3[2][1]=p1->z*p2->y;
       l123->dphidm1[2][2]=rdelta(2,2)*scp;
       l123->dphidm2[2][2]=p1->z*p3->z;
       l123->dphidm3[2][2]=p1->z*p2->z;
/*
 printf("i= %d,%d,%d\n",i1+1,i2+1,i3+1);
 printf("l123= %e\n",l123->dphidm1[0][0]);
 printf("l123= %e\n",l123->dphidm2[0][0]);
 printf("l123= %e\n",l123->dphidm3[0][0]);
 fflush(stdout);
*/
      l123++;
     }
    }
   }

   }

   lb=bptr->idunion.bead.l;
/* I-I matrices */
/*
   for (ib=1,lb1=lb;ib<=nbond;ib++,lb1++) {
    for (jb=ib,lb2=lb1;jb<=nbond;jb++,lb2++) {
*/
   for (ib=0;ib<nbond;ib++) {
    for (jb=ib;jb<nbond;jb++) {
#include "fbb.c"
     scp=0.;
     scp+=f[0][0];
     scp+=f[1][1];
     scp+=f[2][2];
     *objptr->idunion.mcarig.f1++ += weight*scp/3.;
     objptr->idunion.mcarig.Fb[ib][jb].f[0][0]=f[0][0];
     objptr->idunion.mcarig.Fb[ib][jb].f[0][1]=f[0][1];
     objptr->idunion.mcarig.Fb[ib][jb].f[0][2]=f[0][2];
     objptr->idunion.mcarig.Fb[ib][jb].f[1][0]=f[0][1];
     objptr->idunion.mcarig.Fb[ib][jb].f[1][1]=f[1][1];
     objptr->idunion.mcarig.Fb[ib][jb].f[1][2]=f[1][2];
     objptr->idunion.mcarig.Fb[ib][jb].f[2][0]=f[0][2];
     objptr->idunion.mcarig.Fb[ib][jb].f[2][1]=f[1][2];
     objptr->idunion.mcarig.Fb[ib][jb].f[2][2]=f[2][2];

     objptr->idunion.mcarig.Fb[jb][ib].f[0][0]=f[0][0];
     objptr->idunion.mcarig.Fb[jb][ib].f[0][1]=f[0][1];
     objptr->idunion.mcarig.Fb[jb][ib].f[0][2]=f[0][2];
     objptr->idunion.mcarig.Fb[jb][ib].f[1][0]=f[0][1];
     objptr->idunion.mcarig.Fb[jb][ib].f[1][1]=f[1][1];
     objptr->idunion.mcarig.Fb[jb][ib].f[1][2]=f[1][2];
     objptr->idunion.mcarig.Fb[jb][ib].f[2][0]=f[0][2];
     objptr->idunion.mcarig.Fb[jb][ib].f[2][1]=f[1][2];
     objptr->idunion.mcarig.Fb[jb][ib].f[2][2]=f[2][2];

/*
     zr=0.;
#include "fbb0.c"
     scp=0.;
     scp+=f[0][0];
     scp+=f[1][1];
     scp+=f[2][2];
     *objptr->idunion.mcarig.f0++ += scp/3.;
     zr=bptr->idunion.bead.zr;
*/
     scp= lb[ib].x*lb[jb].x+lb[ib].y*lb[jb].y+lb[ib].z*lb[jb].z;
     /* scp *= S_switch[jb-ib]; */
     *objptr->idunion.mcarig.s1++ += weight*scp/3.;
    }
   }
   objptr->idunion.mcarig.s1 -= objptr->idunion.mcarig.dim1;
   objptr->idunion.mcarig.f1 -= objptr->idunion.mcarig.dim1;
/*   objptr->idunion.mcarig.f0 -= objptr->idunion.mcarig.dim1; */


   if(objptr->idunion.mcarig.sel_bonds==0) return;


   lb=bptr->idunion.bead.l;
   l123=objptr->idunion.mcarig.l123;
   for (i1=0,lb1=lb,i=0,kk=0;i1<nbond;i1++,lb1++) {

   j1_0=0;
   j1_1=nbond;
   for (j1=j1_0,j=0,kk=0,l123_2=l123;j1<j1_1;j1++) {
    j2_0=j1-offset_l[j1];
    j2_1=j2_0+doffset[j1];
    for (j2=j2_0,jj=0;j2<j2_1;j2++,jj++) {
     j3_0=j2;
     j3_1=j3_0+doffset[j1]-jj;
     for (j3=j3_0;j3<j3_1;j3++,j++,l123_2++) {

/*
 printf("i1,j1,j2,j3= %d %d %d %d\n",i1+1,j1+1,j2+1,j3+1);
 fflush(stdout);
*/

     for(m1=0;m1<3;m1++) {
      for(m2=0;m2<3;m2++) {
       f[m1][m2]=0.;
      }
     }

     for(m1=0;m1<3;m1++) {
       {
       m2=m1;
       for(k=0;k<3;k++) {
        for(l=0;l<3;l++) {
         f[m1][m2]+=
          rdelta(m1,k)*
          objptr->idunion.mcarig.Fb[i1][j1].f[k][l]  *
          l123_2->dphidm1[m2][l];
         f[m1][m2]+=
          rdelta(m1,k)*
          objptr->idunion.mcarig.Fb[i1][j2].f[k][l]  *
          l123_2->dphidm2[m2][l];
         f[m1][m2]+=
          rdelta(m1,k)*
          objptr->idunion.mcarig.Fb[i1][j3].f[k][l]  *
          l123_2->dphidm3[m2][l];
        }
       }
      }
     }

     scp=0.;
     scp+=f[0][0];
     scp+=f[1][1];
     scp+=f[2][2];
     *objptr->idunion.mcarig.f12 += weight*scp/((_REAL)3);
     objptr->idunion.mcarig.f12++;

     scp=0.;
     scp+=lb1->x*l123_2->phi[0];
     scp+=lb1->y*l123_2->phi[1];
     scp+=lb1->z*l123_2->phi[2];
     *objptr->idunion.mcarig.s12 += weight*scp/((_REAL)3);
     objptr->idunion.mcarig.s12++;

     }
    }
   }

   }
   objptr->idunion.mcarig.s12 -= objptr->idunion.mcarig.dim12;
   objptr->idunion.mcarig.f12 -= objptr->idunion.mcarig.dim12;

   l123=objptr->idunion.mcarig.l123;
   i1_0=0;
   i1_1=nbond;
   for (i1=i1_0,i=0,kk=0,l123_1=l123;i1<i1_1;i1++) {
    i2_0=i1-offset_l[i1];
    i2_1=i2_0+doffset[i1];
    for (i2=i2_0,ii=0;i2<i2_1;i2++,ii++) {
     i3_0=i2;
     i3_1=i3_0+doffset[i1]-ii;
     for (i3=i3_0;i3<i3_1;i3++,i++,l123_1++) {

     j1_0=i1;
     j2_0=i2;
     j3_0=i3;
     j1_1=i1_1;
     j2_1=i2_1;
     j3_1=i3_1;

/*
  printf("i1_0,i2_0,i3_0,j1_0,j2_0,j3_0= %d %d %d %d %d %d\n", 
   i1_0,i2_0,i3_0,j1_0,j2_0,j3_0);
  printf("i1_1,i2_1,i3_1,j1_1,j2_1,j3_1= %d %d %d %d %d %d\n", 
   i1_1,i2_1,i3_1,j1_1,j2_1,j3_1);
*/

   jj0=ii;
   for (j1=j1_0,j=i,l123_2=l123_1;j1<j1_1;j1++,j2_0=j1-offset_l[j1],
    j2_1=j2_0+doffset[j1],j3_0=j2_0,j3_1=j2_1,jj0=0)
   {
    
    for (j2=j2_0,jj=jj0;j2<j2_1;j2++,jj++,j3_0=j2,j3_1=j3_0+doffset[j1]-jj)
    {
/* printf("jj,j3_1= %d %d\n",jj,j3_1); */

     for (j3=j3_0;j3<j3_1;j3++,j++,l123_2++,kk++) 
     {
/*

  printf("i1,i2,i3,j1,j2,j3= %d %d %d %d %d %d\n", 
   i1+1,i2+1,i3+1,j1+1,j2+1,j3+1);
*/
     for(m1=0;m1<3;m1++) {
      for(m2=0;m2<3;m2++) {
       f[m1][m2]=0.;
      }
     }

     for(m1=0;m1<3;m1++) {
       {
       m2=m1;
       for(k=0;k<3;k++) {
        for(l=0;l<3;l++) {
         f[m1][m2]+=
          l123_1->dphidm1[m1][k] *
          objptr->idunion.mcarig.Fb[i1][j1].f[k][l]  *
          l123_2->dphidm1[m2][l];
         f[m1][m2]+=
          l123_1->dphidm1[m1][k] *
          objptr->idunion.mcarig.Fb[i1][j2].f[k][l]  *
          l123_2->dphidm2[m2][l];
         f[m1][m2]+=
          l123_1->dphidm1[m1][k] *
          objptr->idunion.mcarig.Fb[i1][j3].f[k][l]  *
          l123_2->dphidm3[m2][l];
         f[m1][m2]+=
          l123_1->dphidm2[m1][k] *
          objptr->idunion.mcarig.Fb[i2][j1].f[k][l]  *
          l123_2->dphidm1[m2][l];
         f[m1][m2]+=
          l123_1->dphidm2[m1][k] *
          objptr->idunion.mcarig.Fb[i2][j2].f[k][l]  *
          l123_2->dphidm2[m2][l];
         f[m1][m2]+=
          l123_1->dphidm2[m1][k] *
          objptr->idunion.mcarig.Fb[i2][j3].f[k][l]  *
          l123_2->dphidm3[m2][l];
         f[m1][m2]+=
          l123_1->dphidm3[m1][k] *
          objptr->idunion.mcarig.Fb[i3][j1].f[k][l]  *
          l123_2->dphidm1[m2][l];
         f[m1][m2]+=
          l123_1->dphidm3[m1][k] *
          objptr->idunion.mcarig.Fb[i3][j2].f[k][l]  *
          l123_2->dphidm2[m2][l];
         f[m1][m2]+=
          l123_1->dphidm3[m1][k] *
          objptr->idunion.mcarig.Fb[i3][j3].f[k][l]  *
          l123_2->dphidm3[m2][l];
        }
       }
      }
     }

     scp=0.;
     scp+=f[0][0];
     scp+=f[1][1];
     scp+=f[2][2];
     *objptr->idunion.mcarig.f2 += weight*scp/((_REAL)3);
     objptr->idunion.mcarig.f2++;

     scp=0.;
     scp+=l123_1->phi[0]*l123_2->phi[0];
     scp+=l123_1->phi[1]*l123_2->phi[1];
     scp+=l123_1->phi[2]*l123_2->phi[2];
     *objptr->idunion.mcarig.s2 += weight*scp/((_REAL)3);
     objptr->idunion.mcarig.s2++;

     }
    }
   }
     }
    }
   }
   objptr->idunion.mcarig.s2 -= objptr->idunion.mcarig.dim2;
   objptr->idunion.mcarig.f2 -= objptr->idunion.mcarig.dim2;

}

/************************************************************************/

do_first_m1_tens(idstruct *objptr,idstruct *serptr)
{
   _REAL *box=serptr->idunion.ser.set->boxc;
   idstruct *bptr;
   int nbond, *offset_l, *doffset;
   int i,j,kk,l,i1,i2,i3,j1,j2,j3,j1_0,j2_0,j3_0,ib,jb,k,m1,m2,overlap,
       i1_0,i2_0,i3_0,i1_1,i2_1,i3_1,j1_1,j2_1,j3_1,ii,jj,jj0;
   _REAL **r1, **r2;
   _REAL *rt1,*rt2,zi,zj,zr,cutsq;
   _REAL boxm1[3],x1,y1,z1,x2,y2,z2,dx,dy,dz,scp;
   bondstruct *lb1, *lb2, *lb,*p1,*p2,*p3;
   mod123struct *l123,*l123_1,*l123_2;
   _REAL sig0r,f[3][3],weight;
   extern _REAL S_switch[1000];

   sig0r=serptr->idunion.ser.prm->sig0r;
   bptr=objptr->idunion.mcarig.bead;
   zr=bptr->idunion.bead.zr;
   cutsq=bptr->idunion.bead.cutsq;
   nbond=bptr->idunion.bead.nbond;
   offset_l=bptr->idunion.bead.l->offset_l;
   doffset=bptr->idunion.bead.l->doffset;

   for(k=0;k<3;k++) {
    boxm1[k]=1./box[k];
   }

   if(serptr->idunion.ser.iweight)
   {
    weight=serptr->idunion.ser.w->idunion.serw.w;
   } else {
    weight=1.;
   }

   ++objptr->idunion.mcarig.cases;
   objptr->idunion.mcarig.casesw+=weight;

   lb=bptr->idunion.bead.l;
   r1=bptr->idunion.bead.r1;
   r2=bptr->idunion.bead.r2;
   for (i=0;i<bptr->idunion.bead.nbond;i++,r1++,r2++) {
    rt1=*r1; rt2=*r2;
    x1=rt1[0]; y1=rt1[1]; z1=rt1[2];
    x2=rt2[0]; y2=rt2[1]; z2=rt2[2];
/*
   printf("do_first_m1:r1(%d): %e %e %e\n",i+1,sig0r*x1,sig0r*y1,sig0r*z1);
   printf("do_first_m1:r2(%d): %e %e %e\n",i+1,sig0r*x2,sig0r*y2,sig0r*z2);
   fflush(stdout);
*/

    dx=x1-x2;
    dy=y1-y2;
    dz=z1-z2;
    lb->id1 = (int)(*r1);
    lb->id2 = (int)(*r2);
    lb->x1 = x1;
    lb->x2 = x2;
    lb->y1 = y1;
    lb->y2 = y2;
    lb->z1 = z1;
    lb->z2 = z2;
#ifdef PBC
    dx -= box[0]*rint(dx*boxm1[0]);
    dy -= box[1]*rint(dy*boxm1[1]);
    dz -= box[2]*rint(dz*boxm1[2]);
#endif
    lb->x = dx;
    lb->y = dy;
    lb->z = dz;
    lb->sq = dx*dx+dy*dy+dz*dz;
    bptr->idunion.bead.bavt += lb->sq;
    bptr->idunion.bead.bavt2 += lb->sq*lb->sq;
    lb++;
   }
   r1-=bptr->idunion.bead.nbond;
   r2-=bptr->idunion.bead.nbond;

   if(bptr->idunion.bead.bav==0) return;

   if(objptr->idunion.mcarig.sel_bonds!=0)
   {

   l123=objptr->idunion.mcarig.l123;
   lb=bptr->idunion.bead.l;
   for (i1=0,p1=lb;i1<nbond;i1++,p1++) {
    for (i2=0,p2=p1-offset_l[i1];i2<doffset[i1];i2++,p2++) {
     for (i3=i2,p3=p2;i3<doffset[i1];i3++,p3++) {
       scp=p2->x*p3->x+p2->y*p3->y+p2->z*p3->z;
       l123->phi[0]=p1->x*scp;
       l123->phi[1]=p1->y*scp;
       l123->phi[2]=p1->z*scp;
       l123->dphidm1[0][0]=rdelta(0,0)*scp;
       l123->dphidm2[0][0]=p1->x*p3->x;
       l123->dphidm3[0][0]=p1->x*p2->x;
       l123->dphidm1[0][1]=rdelta(0,1)*scp;
       l123->dphidm2[0][1]=p1->x*p3->y;
       l123->dphidm3[0][1]=p1->x*p2->y;
       l123->dphidm1[0][2]=rdelta(0,2)*scp;
       l123->dphidm2[0][2]=p1->x*p3->z;
       l123->dphidm3[0][2]=p1->x*p2->z;
       l123->dphidm1[1][0]=rdelta(1,0)*scp;
       l123->dphidm2[1][0]=p1->y*p3->x;
       l123->dphidm3[1][0]=p1->y*p2->x;
       l123->dphidm1[1][1]=rdelta(1,1)*scp;
       l123->dphidm2[1][1]=p1->y*p3->y;
       l123->dphidm3[1][1]=p1->y*p2->y;
       l123->dphidm1[1][2]=rdelta(1,2)*scp;
       l123->dphidm2[1][2]=p1->y*p3->z;
       l123->dphidm3[1][2]=p1->y*p2->z;
       l123->dphidm1[2][0]=rdelta(2,0)*scp;
       l123->dphidm2[2][0]=p1->z*p3->x;
       l123->dphidm3[2][0]=p1->z*p2->x;
       l123->dphidm1[2][1]=rdelta(2,1)*scp;
       l123->dphidm2[2][1]=p1->z*p3->y;
       l123->dphidm3[2][1]=p1->z*p2->y;
       l123->dphidm1[2][2]=rdelta(2,2)*scp;
       l123->dphidm2[2][2]=p1->z*p3->z;
       l123->dphidm3[2][2]=p1->z*p2->z;
/*
 printf("i= %d,%d,%d\n",i1+1,i2+1,i3+1);
 printf("l123= %e\n",l123->dphidm1[0][0]);
 printf("l123= %e\n",l123->dphidm2[0][0]);
 printf("l123= %e\n",l123->dphidm3[0][0]);
 fflush(stdout);
*/
      l123++;
     }
    }
   }

   }

   lb=bptr->idunion.bead.l;
/* I-I matrices */
   for (ib=1,lb1=lb;ib<=nbond;ib++,lb1++) {
    for (jb=ib,lb2=lb1;jb<=nbond;jb++,lb2++) {
#include "fbb.c"
     f[1][0]=f[0][1];
     f[2][0]=f[0][2];
     f[2][1]=f[1][2];
     objptr->idunion.mcarig.ft1->f[0][0] += weight*f[0][0];
     objptr->idunion.mcarig.ft1->f[0][1] += weight*f[0][1];
     objptr->idunion.mcarig.ft1->f[0][2] += weight*f[0][2];
     objptr->idunion.mcarig.ft1->f[1][0] += weight*f[1][0];
     objptr->idunion.mcarig.ft1->f[1][1] += weight*f[1][1];
     objptr->idunion.mcarig.ft1->f[1][2] += weight*f[1][2];
     objptr->idunion.mcarig.ft1->f[2][0] += weight*f[2][0];
     objptr->idunion.mcarig.ft1->f[2][1] += weight*f[2][1];
     objptr->idunion.mcarig.ft1->f[2][2] += weight*f[2][2];
     objptr->idunion.mcarig.Fb[ib-1][jb-1].f[0][0]=f[0][0];
     objptr->idunion.mcarig.Fb[ib-1][jb-1].f[0][1]=f[0][1];
     objptr->idunion.mcarig.Fb[ib-1][jb-1].f[0][2]=f[0][2];
     objptr->idunion.mcarig.Fb[ib-1][jb-1].f[1][0]=f[1][0];
     objptr->idunion.mcarig.Fb[ib-1][jb-1].f[1][1]=f[1][1];
     objptr->idunion.mcarig.Fb[ib-1][jb-1].f[1][2]=f[1][2];
     objptr->idunion.mcarig.Fb[ib-1][jb-1].f[2][0]=f[2][0];
     objptr->idunion.mcarig.Fb[ib-1][jb-1].f[2][1]=f[2][1];
     objptr->idunion.mcarig.Fb[ib-1][jb-1].f[2][2]=f[2][2];

     objptr->idunion.mcarig.Fb[jb-1][ib-1].f[0][0]=f[0][0];
     objptr->idunion.mcarig.Fb[jb-1][ib-1].f[0][1]=f[1][0];
     objptr->idunion.mcarig.Fb[jb-1][ib-1].f[0][2]=f[2][0];
     objptr->idunion.mcarig.Fb[jb-1][ib-1].f[1][0]=f[0][1];
     objptr->idunion.mcarig.Fb[jb-1][ib-1].f[1][1]=f[1][1];
     objptr->idunion.mcarig.Fb[jb-1][ib-1].f[1][2]=f[2][1];
     objptr->idunion.mcarig.Fb[jb-1][ib-1].f[2][0]=f[0][2];
     objptr->idunion.mcarig.Fb[jb-1][ib-1].f[2][1]=f[1][2];
     objptr->idunion.mcarig.Fb[jb-1][ib-1].f[2][2]=f[2][2];

     scp= lb1->x*lb2->x;
     objptr->idunion.mcarig.st1->f[0][0] += weight*scp;
     scp= lb1->x*lb2->y;
     objptr->idunion.mcarig.st1->f[0][1] += weight*scp;
     scp= lb1->x*lb2->z;
     objptr->idunion.mcarig.st1->f[0][2] += weight*scp;
     scp= lb1->y*lb2->x;
     objptr->idunion.mcarig.st1->f[1][0] += weight*scp;
     scp= lb1->y*lb2->y;
     objptr->idunion.mcarig.st1->f[1][1] += weight*scp;
     scp= lb1->y*lb2->z;
     objptr->idunion.mcarig.st1->f[1][2] += weight*scp;
     scp= lb1->z*lb2->x;
     objptr->idunion.mcarig.st1->f[2][0] += weight*scp;
     scp= lb1->z*lb2->y;
     objptr->idunion.mcarig.st1->f[2][1] += weight*scp;
     scp= lb1->z*lb2->z;
     objptr->idunion.mcarig.st1->f[2][2] += weight*scp;
     objptr->idunion.mcarig.st1++;
     objptr->idunion.mcarig.ft1++;
 
    }
   }
   objptr->idunion.mcarig.st1 -= objptr->idunion.mcarig.dim1;
   objptr->idunion.mcarig.ft1 -= objptr->idunion.mcarig.dim1;


   if(objptr->idunion.mcarig.sel_bonds==0) return;


   lb=bptr->idunion.bead.l;
   l123=objptr->idunion.mcarig.l123;
   for (i1=0,lb1=lb,i=0,kk=0;i1<nbond;i1++,lb1++) {

   j1_0=0;
   j1_1=nbond;
   for (j1=j1_0,j=0,kk=0,l123_2=l123;j1<j1_1;j1++) {
    j2_0=j1-offset_l[j1];
    j2_1=j2_0+doffset[j1];
    for (j2=j2_0,jj=0;j2<j2_1;j2++,jj++) {
     j3_0=j2;
     j3_1=j3_0+doffset[j1]-jj;
     for (j3=j3_0;j3<j3_1;j3++,j++,l123_2++) {

/*
 printf("i1,j1,j2,j3= %d %d %d %d\n",i1+1,j1+1,j2+1,j3+1);
 fflush(stdout);
*/

     for(m1=0;m1<3;m1++) {
      for(m2=0;m2<3;m2++) {
       f[m1][m2]=0.;
      }
     }

     for(m1=0;m1<3;m1++)
     {
      for(m2=0;m2<3;m2++)
      {
       for(k=0;k<3;k++) {
        for(l=0;l<3;l++) {
         f[m1][m2]+=
          rdelta(m1,k)*
          objptr->idunion.mcarig.Fb[i1][j1].f[k][l]  *
          l123_2->dphidm1[m2][l];
         f[m1][m2]+=
          rdelta(m1,k)*
          objptr->idunion.mcarig.Fb[i1][j2].f[k][l]  *
          l123_2->dphidm2[m2][l];
         f[m1][m2]+=
          rdelta(m1,k)*
          objptr->idunion.mcarig.Fb[i1][j3].f[k][l]  *
          l123_2->dphidm3[m2][l];
	 scp=f[m1][m2];
         objptr->idunion.mcarig.ft12->f[m1][m2] += weight*scp;
/*
            printf("i1,j1,j2,j3,m1,m2,k,l: %d %d %d %d %d %d %d %d\n",
             i1,j1,j2,j3,m1,m2,k,l); 
            printf("%e %e %e %e %e %e %e ",
             rdelta(m1,k),
             objptr->idunion.mcarig.Fb[i1][j1].f[k][l],
	     l123_2->dphidm1[m2][l],
             objptr->idunion.mcarig.Fb[i1][j2].f[k][l],
             l123_2->dphidm2[m2][l],
             objptr->idunion.mcarig.Fb[i1][j3].f[k][l],
             l123_2->dphidm3[m2][l]);
            printf("%e\n",
             objptr->idunion.mcarig.ft12->f[m1][m2]);
*/
        }
       }
      }
     }
     scp= lb1->x*l123_2->phi[0];
     objptr->idunion.mcarig.st12->f[0][0] += weight*scp;
     scp= lb1->x*l123_2->phi[1];
     objptr->idunion.mcarig.st12->f[0][1] += weight*scp;
     scp= lb1->x*l123_2->phi[2];
     objptr->idunion.mcarig.st12->f[0][2] += weight*scp;
     scp= lb1->y*l123_2->phi[0];
     objptr->idunion.mcarig.st12->f[1][0] += weight*scp;
     scp= lb1->y*l123_2->phi[1];
     objptr->idunion.mcarig.st12->f[1][1] += weight*scp;
     scp= lb1->y*l123_2->phi[2];
     objptr->idunion.mcarig.st12->f[1][2] += weight*scp;
     scp= lb1->z*l123_2->phi[0];
     objptr->idunion.mcarig.st12->f[2][0] += weight*scp;
     scp= lb1->z*l123_2->phi[1];
     objptr->idunion.mcarig.st12->f[2][1] += weight*scp;
     scp= lb1->z*l123_2->phi[2];
     objptr->idunion.mcarig.st12->f[2][2] += weight*scp;
     objptr->idunion.mcarig.st12++;
     objptr->idunion.mcarig.ft12++;

     }
    }
   }

   }
   objptr->idunion.mcarig.st12 -= objptr->idunion.mcarig.dim12;
   objptr->idunion.mcarig.ft12 -= objptr->idunion.mcarig.dim12;

   l123=objptr->idunion.mcarig.l123;
   i1_0=0;
   i1_1=nbond;
   for (i1=i1_0,i=0,kk=0,l123_1=l123;i1<i1_1;i1++) {
    i2_0=i1-offset_l[i1];
    i2_1=i2_0+doffset[i1];
    for (i2=i2_0,ii=0;i2<i2_1;i2++,ii++) {
     i3_0=i2;
     i3_1=i3_0+doffset[i1]-ii;
     for (i3=i3_0;i3<i3_1;i3++,i++,l123_1++) {

     j1_0=i1;
     j2_0=i2;
     j3_0=i3;
     j1_1=i1_1;
     j2_1=i2_1;
     j3_1=i3_1;

/*
  printf("i1_0,i2_0,i3_0,j1_0,j2_0,j3_0= %d %d %d %d %d %d\n", 
   i1_0,i2_0,i3_0,j1_0,j2_0,j3_0);
  printf("i1_1,i2_1,i3_1,j1_1,j2_1,j3_1= %d %d %d %d %d %d\n", 
   i1_1,i2_1,i3_1,j1_1,j2_1,j3_1);
*/

   jj0=ii;
   for (j1=j1_0,j=i,l123_2=l123_1;j1<j1_1;j1++,j2_0=j1-offset_l[j1],
    j2_1=j2_0+doffset[j1],j3_0=j2_0,j3_1=j2_1,jj0=0)
   {
    
    for (j2=j2_0,jj=jj0;j2<j2_1;j2++,jj++,j3_0=j2,j3_1=j3_0+doffset[j1]-jj)
    {
/* printf("jj,j3_1= %d %d\n",jj,j3_1); */

     for (j3=j3_0;j3<j3_1;j3++,j++,l123_2++,kk++) 
     {
/*

  printf("i1,i2,i3,j1,j2,j3= %d %d %d %d %d %d\n", 
   i1+1,i2+1,i3+1,j1+1,j2+1,j3+1);
*/
     for(m1=0;m1<3;m1++) {
      for(m2=0;m2<3;m2++) {
       f[m1][m2]=0.;
      }
     }

     for(m1=0;m1<3;m1++)
     {
      for(m2=0;m2<3;m2++)
      {
       for(k=0;k<3;k++) {
        for(l=0;l<3;l++) {
         f[m1][m2]+=
          l123_1->dphidm1[m1][k] *
          objptr->idunion.mcarig.Fb[i1][j1].f[k][l]  *
          l123_2->dphidm1[m2][l];
         f[m1][m2]+=
          l123_1->dphidm1[m1][k] *
          objptr->idunion.mcarig.Fb[i1][j2].f[k][l]  *
          l123_2->dphidm2[m2][l];
         f[m1][m2]+=
          l123_1->dphidm1[m1][k] *
          objptr->idunion.mcarig.Fb[i1][j3].f[k][l]  *
          l123_2->dphidm3[m2][l];
         f[m1][m2]+=
          l123_1->dphidm2[m1][k] *
          objptr->idunion.mcarig.Fb[i2][j1].f[k][l]  *
          l123_2->dphidm1[m2][l];
         f[m1][m2]+=
          l123_1->dphidm2[m1][k] *
          objptr->idunion.mcarig.Fb[i2][j2].f[k][l]  *
          l123_2->dphidm2[m2][l];
         f[m1][m2]+=
          l123_1->dphidm2[m1][k] *
          objptr->idunion.mcarig.Fb[i2][j3].f[k][l]  *
          l123_2->dphidm3[m2][l];
         f[m1][m2]+=
          l123_1->dphidm3[m1][k] *
          objptr->idunion.mcarig.Fb[i3][j1].f[k][l]  *
          l123_2->dphidm1[m2][l];
         f[m1][m2]+=
          l123_1->dphidm3[m1][k] *
          objptr->idunion.mcarig.Fb[i3][j2].f[k][l]  *
          l123_2->dphidm2[m2][l];
         f[m1][m2]+=
          l123_1->dphidm3[m1][k] *
          objptr->idunion.mcarig.Fb[i3][j3].f[k][l]  *
          l123_2->dphidm3[m2][l];
	 scp=f[m1][m2];
         objptr->idunion.mcarig.ft2->f[m1][m2] += weight*scp;
        }
       }
      }
     }
     scp= l123_1->phi[0]*l123_2->phi[0];
     objptr->idunion.mcarig.st2->f[0][0] += weight*scp;
     scp= l123_1->phi[0]*l123_2->phi[1];
     objptr->idunion.mcarig.st2->f[0][1] += weight*scp;
     scp= l123_1->phi[0]*l123_2->phi[2];
     objptr->idunion.mcarig.st2->f[0][2] += weight*scp;
     scp= l123_1->phi[1]*l123_2->phi[0];
     objptr->idunion.mcarig.st2->f[1][0] += weight*scp;
     scp= l123_1->phi[1]*l123_2->phi[1];
     objptr->idunion.mcarig.st2->f[1][1] += weight*scp;
     scp= l123_1->phi[1]*l123_2->phi[2];
     objptr->idunion.mcarig.st2->f[1][2] += weight*scp;
     scp= l123_1->phi[2]*l123_2->phi[0];
     objptr->idunion.mcarig.st2->f[2][0] += weight*scp;
     scp= l123_1->phi[2]*l123_2->phi[1];
     objptr->idunion.mcarig.st2->f[2][1] += weight*scp;
     scp= l123_1->phi[2]*l123_2->phi[2];
     objptr->idunion.mcarig.st2->f[2][2] += weight*scp;
     objptr->idunion.mcarig.st2++;
     objptr->idunion.mcarig.ft2++;

     }
    }
   }
     }
    }
   }
   objptr->idunion.mcarig.st2 -= objptr->idunion.mcarig.dim2;
   objptr->idunion.mcarig.ft2 -= objptr->idunion.mcarig.dim2;

 printf("sto uscendo da do_first_m1...\n"); fflush(stdout);
}

/************************************************************************/

do_first_m1_list(idstruct *objptr,idstruct *serptr)
{
   _REAL *box=serptr->idunion.ser.set->boxc;
   idstruct *bptr;
   int nbond,nlist;
   int i,j,kk,l,i1,i2,i3,j1,j2,j3,j1_0,j2_0,j3_0,ib,jb,k,m1,m2,overlap,
       i1_0,i2_0,i3_0,i1_1,i2_1,i3_1,j1_1,j2_1,j3_1,ii,jj,jj0;
   _REAL **r1, **r2;
   _REAL *rt1,*rt2,zi,zj,zr,cutsq;
   _REAL boxm1[3],x1,y1,z1,x2,y2,z2,dx,dy,dz,scp;
   bondstruct *lb1, *lb2, *lb,p1,p2,p3;
   mod123struct *l123,*l123_1,*l123_2;
   _REAL sig0r,f[3][3],weight;

   sig0r=serptr->idunion.ser.prm->sig0r;
   bptr=objptr->idunion.mcarig.bead;
   zr=bptr->idunion.bead.zr;
   cutsq=bptr->idunion.bead.cutsq;
   nbond=bptr->idunion.bead.nbond;
   nlist=objptr->idunion.mcarig.nlist;

   for(k=0;k<3;k++) {
    boxm1[k]=1./box[k];
   }

/*
    printf("do_first_m1_list: nbond: %d\n",bptr->idunion.bead.nbond);
    printf("do_first_m1_list: nbond: %d\n",objptr->idunion.mcarig.nlist);
    fflush(stdout);
*/

   if(serptr->idunion.ser.iweight)
   {
    weight=serptr->idunion.ser.w->idunion.serw.w;
   } else {
    weight=1.;
   }

   ++objptr->idunion.mcarig.cases;
   objptr->idunion.mcarig.casesw+=weight;

   lb=bptr->idunion.bead.l;
   r1=bptr->idunion.bead.r1;
   r2=bptr->idunion.bead.r2;
   for (i=0;i<bptr->idunion.bead.nbond;i++,r1++,r2++) {
    rt1=*r1; rt2=*r2;
    x1=rt1[0]; y1=rt1[1]; z1=rt1[2];
    x2=rt2[0]; y2=rt2[1]; z2=rt2[2];
/*
   printf("do_first_m1:r1(%d): %e %e %e\n",i+1,sig0r*x1,sig0r*y1,sig0r*z1);
   printf("do_first_m1:r2(%d): %e %e %e\n",i+1,sig0r*x2,sig0r*y2,sig0r*z2);
   fflush(stdout);
*/

    dx=x1-x2;
    dy=y1-y2;
    dz=z1-z2;
    lb->id1 = (int)(*r1);
    lb->id2 = (int)(*r2);
    lb->x1 = x1;
    lb->x2 = x2;
    lb->y1 = y1;
    lb->y2 = y2;
    lb->z1 = z1;
    lb->z2 = z2;
#ifdef PBC
    dx -= box[0]*rint(dx*boxm1[0]);
    dy -= box[1]*rint(dy*boxm1[1]);
    dz -= box[2]*rint(dz*boxm1[2]);
#endif
    lb->x = dx;
    lb->y = dy;
    lb->z = dz;
    lb->sq = dx*dx+dy*dy+dz*dz;
    bptr->idunion.bead.bavt += lb->sq;
    bptr->idunion.bead.bavt2 += lb->sq*lb->sq;
    lb++;
   }
   r1-=bptr->idunion.bead.nbond;
   r2-=bptr->idunion.bead.nbond;

   if(objptr->idunion.mcarig.sel_bonds!=0)
   {

   l123=objptr->idunion.mcarig.l123;
   lb=bptr->idunion.bead.l;
   for (i=0;i<nlist;i++) {
    i1=objptr->idunion.mcarig.j1[i];
    i2=objptr->idunion.mcarig.j2[i];
    i3=objptr->idunion.mcarig.j3[i];
    p1=bptr->idunion.bead.l[i1];
    p2=bptr->idunion.bead.l[i2];
    p3=bptr->idunion.bead.l[i3];
       scp=p2.x*p3.x+p2.y*p3.y+p2.z*p3.z;
       l123->phi[0]=p1.x*scp;
       l123->phi[1]=p1.y*scp;
       l123->phi[2]=p1.z*scp;
       l123->dphidm1[0][0]=rdelta(0,0)*scp;
       l123->dphidm2[0][0]=p1.x*p3.x;
       l123->dphidm3[0][0]=p1.x*p2.x;
       l123->dphidm1[0][1]=rdelta(0,1)*scp;
       l123->dphidm2[0][1]=p1.x*p3.y;
       l123->dphidm3[0][1]=p1.x*p2.y;
       l123->dphidm1[0][2]=rdelta(0,2)*scp;
       l123->dphidm2[0][2]=p1.x*p3.z;
       l123->dphidm3[0][2]=p1.x*p2.z;
       l123->dphidm1[1][0]=rdelta(1,0)*scp;
       l123->dphidm2[1][0]=p1.y*p3.x;
       l123->dphidm3[1][0]=p1.y*p2.x;
       l123->dphidm1[1][1]=rdelta(1,1)*scp;
       l123->dphidm2[1][1]=p1.y*p3.y;
       l123->dphidm3[1][1]=p1.y*p2.y;
       l123->dphidm1[1][2]=rdelta(1,2)*scp;
       l123->dphidm2[1][2]=p1.y*p3.z;
       l123->dphidm3[1][2]=p1.y*p2.z;
       l123->dphidm1[2][0]=rdelta(2,0)*scp;
       l123->dphidm2[2][0]=p1.z*p3.x;
       l123->dphidm3[2][0]=p1.z*p2.x;
       l123->dphidm1[2][1]=rdelta(2,1)*scp;
       l123->dphidm2[2][1]=p1.z*p3.y;
       l123->dphidm3[2][1]=p1.z*p2.y;
       l123->dphidm1[2][2]=rdelta(2,2)*scp;
       l123->dphidm2[2][2]=p1.z*p3.z;
       l123->dphidm3[2][2]=p1.z*p2.z;
/*
 printf("i= %d,%d,%d\n",i1+1,i2+1,i3+1);
 fflush(stdout);
*/
      l123++;
   }

   }

   lb=bptr->idunion.bead.l;
/* I-I matrices */
   for (ib=1,lb1=lb;ib<=nbond;ib++,lb1++) {
    for (jb=ib,lb2=lb1;jb<=nbond;jb++,lb2++) {

#include "fbb.c"
     scp=0.;
     scp+=f[0][0];
     scp+=f[1][1];
     scp+=f[2][2];
     *objptr->idunion.mcarig.f1++ += weight*scp/3.;
     objptr->idunion.mcarig.Fb[ib-1][jb-1].f[0][0]=f[0][0];
     objptr->idunion.mcarig.Fb[ib-1][jb-1].f[0][1]=f[0][1];
     objptr->idunion.mcarig.Fb[ib-1][jb-1].f[0][2]=f[0][2];
     objptr->idunion.mcarig.Fb[ib-1][jb-1].f[1][0]=f[0][1];
     objptr->idunion.mcarig.Fb[ib-1][jb-1].f[1][1]=f[1][1];
     objptr->idunion.mcarig.Fb[ib-1][jb-1].f[1][2]=f[1][2];
     objptr->idunion.mcarig.Fb[ib-1][jb-1].f[2][0]=f[0][2];
     objptr->idunion.mcarig.Fb[ib-1][jb-1].f[2][1]=f[1][2];
     objptr->idunion.mcarig.Fb[ib-1][jb-1].f[2][2]=f[2][2];

     objptr->idunion.mcarig.Fb[jb-1][ib-1].f[0][0]=f[0][0];
     objptr->idunion.mcarig.Fb[jb-1][ib-1].f[0][1]=f[0][1];
     objptr->idunion.mcarig.Fb[jb-1][ib-1].f[0][2]=f[0][2];
     objptr->idunion.mcarig.Fb[jb-1][ib-1].f[1][0]=f[0][1];
     objptr->idunion.mcarig.Fb[jb-1][ib-1].f[1][1]=f[1][1];
     objptr->idunion.mcarig.Fb[jb-1][ib-1].f[1][2]=f[1][2];
     objptr->idunion.mcarig.Fb[jb-1][ib-1].f[2][0]=f[0][2];
     objptr->idunion.mcarig.Fb[jb-1][ib-1].f[2][1]=f[1][2];
     objptr->idunion.mcarig.Fb[jb-1][ib-1].f[2][2]=f[2][2];

/*
     zr=0.;
#include "fbb0.c"
     scp=0.;
     scp+=f[0][0];
     scp+=f[1][1];
     scp+=f[2][2];
     *objptr->idunion.mcarig.f0++ += scp/3.;
     zr=bptr->idunion.bead.zr;
*/
     scp= lb1->x*lb2->x+lb1->y*lb2->y+lb1->z*lb2->z;
     /* scp *= S_switch[jb-ib]; */
     *objptr->idunion.mcarig.s1++ += weight*scp/3.;

    }
   }
   objptr->idunion.mcarig.s1 -= objptr->idunion.mcarig.dim1;
   objptr->idunion.mcarig.f1 -= objptr->idunion.mcarig.dim1;
/*   objptr->idunion.mcarig.f0 -= objptr->idunion.mcarig.dim1; */


   if(objptr->idunion.mcarig.sel_bonds==0) return;


   lb=bptr->idunion.bead.l;
   l123=objptr->idunion.mcarig.l123;
   for (i1=0,lb1=lb,kk=0;i1<nbond;i1++,lb1++) {

   for (j=0,l123_2=l123;j<nlist;j++,l123_2++,kk++) {
    j1=objptr->idunion.mcarig.j1[j];
    j2=objptr->idunion.mcarig.j2[j];
    j3=objptr->idunion.mcarig.j3[j];

/*
 printf("kk,i1,j1,j2,j3= %d %d %d %d %d\n",kk,i1+1,j1+1,j2+1,j3+1);
 fflush(stdout);
*/

     for(m1=0;m1<3;m1++) {
      for(m2=0;m2<3;m2++) {
       f[m1][m2]=0.;
      }
     }

     for(m1=0;m1<3;m1++) {
       {
       m2=m1;
       for(k=0;k<3;k++) {
        for(l=0;l<3;l++) {
         f[m1][m2]+=
          rdelta(m1,k)*
          objptr->idunion.mcarig.Fb[i1][j1].f[k][l]  *
          l123_2->dphidm1[m2][l];
         f[m1][m2]+=
          rdelta(m1,k)*
          objptr->idunion.mcarig.Fb[i1][j2].f[k][l]  *
          l123_2->dphidm2[m2][l];
         f[m1][m2]+=
          rdelta(m1,k)*
          objptr->idunion.mcarig.Fb[i1][j3].f[k][l]  *
          l123_2->dphidm3[m2][l];
        }
       }
      }
     }

     scp=0.;
     scp+=f[0][0];
     scp+=f[1][1];
     scp+=f[2][2];
     *objptr->idunion.mcarig.f12 += weight*scp/((_REAL)3);
     objptr->idunion.mcarig.f12++;

     scp=0.;
     scp+=lb1->x*l123_2->phi[0];
     scp+=lb1->y*l123_2->phi[1];
     scp+=lb1->z*l123_2->phi[2];
     *objptr->idunion.mcarig.s12 += weight*scp/((_REAL)3);
     objptr->idunion.mcarig.s12++;

   }

   }
   objptr->idunion.mcarig.s12 -= objptr->idunion.mcarig.dim12;
   objptr->idunion.mcarig.f12 -= objptr->idunion.mcarig.dim12;

   l123=objptr->idunion.mcarig.l123;
   for (i=0,kk=0,l123_1=l123;i<nlist;i++,l123_1++) {
    i1=objptr->idunion.mcarig.j1[i];
    i2=objptr->idunion.mcarig.j2[i];
    i3=objptr->idunion.mcarig.j3[i];

   for (j=i,l123_2=l123_1;j<nlist;j++,l123_2++,kk++)
   {
    j1=objptr->idunion.mcarig.j1[j];
    j2=objptr->idunion.mcarig.j2[j];
    j3=objptr->idunion.mcarig.j3[j];
    
/*
  printf("i1,i2,i3,j1,j2,j3= %d %d %d %d %d %d\n", 
   i1+1,i2+1,i3+1,j1+1,j2+1,j3+1);
*/

     for(m1=0;m1<3;m1++) {
      for(m2=0;m2<3;m2++) {
       f[m1][m2]=0.;
      }
     }

     for(m1=0;m1<3;m1++) {
       {
       m2=m1;
       for(k=0;k<3;k++) {
        for(l=0;l<3;l++) {
         f[m1][m2]+=
          l123_1->dphidm1[m1][k] *
          objptr->idunion.mcarig.Fb[i1][j1].f[k][l]  *
          l123_2->dphidm1[m2][l];
         f[m1][m2]+=
          l123_1->dphidm1[m1][k] *
          objptr->idunion.mcarig.Fb[i1][j2].f[k][l]  *
          l123_2->dphidm2[m2][l];
         f[m1][m2]+=
          l123_1->dphidm1[m1][k] *
          objptr->idunion.mcarig.Fb[i1][j3].f[k][l]  *
          l123_2->dphidm3[m2][l];
         f[m1][m2]+=
          l123_1->dphidm2[m1][k] *
          objptr->idunion.mcarig.Fb[i2][j1].f[k][l]  *
          l123_2->dphidm1[m2][l];
         f[m1][m2]+=
          l123_1->dphidm2[m1][k] *
          objptr->idunion.mcarig.Fb[i2][j2].f[k][l]  *
          l123_2->dphidm2[m2][l];
         f[m1][m2]+=
          l123_1->dphidm2[m1][k] *
          objptr->idunion.mcarig.Fb[i2][j3].f[k][l]  *
          l123_2->dphidm3[m2][l];
         f[m1][m2]+=
          l123_1->dphidm3[m1][k] *
          objptr->idunion.mcarig.Fb[i3][j1].f[k][l]  *
          l123_2->dphidm1[m2][l];
         f[m1][m2]+=
          l123_1->dphidm3[m1][k] *
          objptr->idunion.mcarig.Fb[i3][j2].f[k][l]  *
          l123_2->dphidm2[m2][l];
         f[m1][m2]+=
          l123_1->dphidm3[m1][k] *
          objptr->idunion.mcarig.Fb[i3][j3].f[k][l]  *
          l123_2->dphidm3[m2][l];
        }
       }
      }
     }

     scp=0.;
     scp+=f[0][0];
     scp+=f[1][1];
     scp+=f[2][2];
     *objptr->idunion.mcarig.f2 += weight*scp/((_REAL)3);
     objptr->idunion.mcarig.f2++;

     scp=0.;
     scp+=l123_1->phi[0]*l123_2->phi[0];
     scp+=l123_1->phi[1]*l123_2->phi[1];
     scp+=l123_1->phi[2]*l123_2->phi[2];
     *objptr->idunion.mcarig.s2 += weight*scp/((_REAL)3);
     objptr->idunion.mcarig.s2++;

    }
   }
   objptr->idunion.mcarig.s2 -= objptr->idunion.mcarig.dim2;
   objptr->idunion.mcarig.f2 -= objptr->idunion.mcarig.dim2;

}


/************************************************************************/

void
write_first(idstruct *dptr)
{
	int	i,j,k,l,m,i2,j2,i3,j3,bytes;
	int	states = dptr->idunion.mcarig.cases;
	FILE 	*file = dptr->idunion.mcarig.file,*dump;
        idstruct *bptr=dptr->idunion.mcarig.bead;

	_REAL fact;

        int n1,n2,nn,nf,rank,mm,m1,m2,m2_0;
        _REAL **F,**S,**Fm1,*w,**tmp,**F0,**tmp0;
        _REAL sum,sum1,sum2,tol,RKR;
        _REAL *u,*v;
#ifdef IMSL
        d_complex *vi,*wi;
#else
        _REAL *vi;
#endif

	/* fact = 1./ ((_REAL)states); */
	fact = 1./ dptr->idunion.mcarig.casesw;

        printf("write_first: number of states: %d\n",states);

        n1 = dptr->idunion.mcarig.n1;
        n2 = dptr->idunion.mcarig.n2;
        nn = dptr->idunion.mcarig.nn;
        dptr->idunion.mcarig.evec = matrix( 1,nn, 1,nn);
/*
        for(i=1,sum=0.;i<=bptr->idunion.bead.nbead-1;i++) {
         for(j=i+1;j<=bptr->idunion.bead.nbead;j++) {
          dptr->idunion.mcarig.Rm1[i][j] *= fact;
          sum+=2.*bptr->idunion.bead.z[i-1]*bptr->idunion.bead.z[j-1]*
                  dptr->idunion.mcarig.Rm1[i][j];
         }
        }
        RKR = ((_REAL)bptr->idunion.bead.nbead)*bptr->idunion.bead.zav/
          (1. + (4./3.)*zr*sum/
                 ((_REAL)bptr->idunion.bead.nbead)/bptr->idunion.bead.zav );
        printf("RKR= %e\n",RKR);
        exit(0);
*/


	if(dptr->idunion.mcarig.dump==-1) 
	{
         dump=fopen("../evec_m1.dump","r");
         for(i=1;i<=nn;i++) 
	 {
          for(j=1;j<=nn;j++) 
	  {
           fscanf(dump,"%le",&dptr->idunion.mcarig.evec[i][j]);
	  }
	 }
         fclose(dump);
	 printf("write_first: obtained rank-1 eigenvectors from file.\n");
	 printf("write_first: last evec element: evec(%d,%d)= %e\n",
          nn,nn,dptr->idunion.mcarig.evec[nn][nn]);

	 return(0);
        }

        if(bptr->idunion.bead.bav==0.) {
         bptr->idunion.bead.bav =
          bptr->idunion.bead.bavt*fact/bptr->idunion.bead.nbond;
         bptr->idunion.bead.bav = bptr->idunion.bead.prm->sig0r*
          l_sqrt(bptr->idunion.bead.bav);
         printf("write_first: bav: %e\n",bptr->idunion.bead.bav);
         bptr->idunion.bead.bav =
          bptr->idunion.bead.bavt2*fact/bptr->idunion.bead.nbond;
         bptr->idunion.bead.bav =
          bptr->idunion.bead.prm->sig0r*
          bptr->idunion.bead.prm->sig0r*
          l_sqrt(bptr->idunion.bead.bav);
         printf("write_first: bav2: %e\n",bptr->idunion.bead.bav);
         exit(0);
        }

       if(dptr->idunion.mcarig.tens==0)
       {
        nn = dptr->idunion.mcarig.nn;
        F = matrix(1,nn,1,nn);
        S = matrix(1,nn,1,nn);

//        sum=0.;
        for (i=1; i<=n1; i++) {
         for (j=i; j<=n1; j++) {
           *dptr->idunion.mcarig.s1 *= fact;
           *dptr->idunion.mcarig.f1 *= fact;
           S[i][j]=(*dptr->idunion.mcarig.s1);
/*
           S[i][j]=(1./3.)*(_REAL)idelta(i,j);
           *dptr->idunion.mcarig.s1=S[i][j];
*/
           S[j][i]=S[i][j];
           F[i][j]=(*dptr->idunion.mcarig.f1);
           F[j][i]=F[i][j];
           dptr->idunion.mcarig.s1++;
// printf("%d %d %e\n",i,j,S[i][j]);
           dptr->idunion.mcarig.f1++;
         }
//        sum+=S[i][i];
        }
        dptr->idunion.mcarig.s1 -= dptr->idunion.mcarig.dim1;
        dptr->idunion.mcarig.f1 -= dptr->idunion.mcarig.dim1;
//        printf("write_first: trace of S= %e\n",sum);

        if(dptr->idunion.mcarig.sel_bonds!=0)
        {

        for (i=1; i<=n1; i++) {
         for (j=1; j<=n2; j++) {
           *dptr->idunion.mcarig.s12 *= fact;
           *dptr->idunion.mcarig.f12 *= fact;
           j2=n1+j;
           S[i][j2]=*dptr->idunion.mcarig.s12;
           S[j2][i]=S[i][j2];
           F[i][j2]=*dptr->idunion.mcarig.f12;
           F[j2][i]=F[i][j2];
           dptr->idunion.mcarig.s12++;
           dptr->idunion.mcarig.f12++;
         }
        }
        dptr->idunion.mcarig.s12 -= dptr->idunion.mcarig.dim12;
        dptr->idunion.mcarig.f12 -= dptr->idunion.mcarig.dim12;

        for (i=1; i<=n2; i++) {
         for (j=i; j<=n2; j++) {
           *dptr->idunion.mcarig.s2 *= fact;
           *dptr->idunion.mcarig.f2 *= fact;
           i2=n1+i;
           j2=n1+j;
           S[i2][j2]=*dptr->idunion.mcarig.s2;
           S[j2][i2]=S[i2][j2];
           F[i2][j2]=*dptr->idunion.mcarig.f2;
           F[j2][i2]=F[i2][j2];
           dptr->idunion.mcarig.s2++;
           dptr->idunion.mcarig.f2++;
         }
        }
        dptr->idunion.mcarig.s2 -= dptr->idunion.mcarig.dim2;
        dptr->idunion.mcarig.f2 -= dptr->idunion.mcarig.dim2;

        }

        if(dptr->idunion.mcarig.dump==1) {
         dump=fopen("fs.dump","w");
 	 bytes=sizeof(_REAL)*dptr->idunion.mcarig.dim1;
         fwrite(&bytes,sizeof(int),1,dump);
         fwrite(dptr->idunion.mcarig.f1,dptr->idunion.mcarig.dim1*sizeof(_REAL),
                1,dump);
         fwrite(&bytes,sizeof(int),1,dump);
         fwrite(&bytes,sizeof(int),1,dump);
         fwrite(dptr->idunion.mcarig.s1,dptr->idunion.mcarig.dim1*sizeof(_REAL),
                1,dump);
         fwrite(&bytes,sizeof(int),1,dump);
         if(dptr->idunion.mcarig.n2>0)
	 {
   	  bytes=sizeof(_REAL)*dptr->idunion.mcarig.dim12;
          fwrite(&bytes,sizeof(int),1,dump);
          fwrite(dptr->idunion.mcarig.f12,dptr->idunion.mcarig.dim12*sizeof(_REAL),
                 1,dump);
          fwrite(&bytes,sizeof(int),1,dump);
          fwrite(&bytes,sizeof(int),1,dump);
          fwrite(dptr->idunion.mcarig.s12,dptr->idunion.mcarig.dim12*sizeof(_REAL),
                 1,dump);
          fwrite(&bytes,sizeof(int),1,dump);
 	  bytes=sizeof(_REAL)*dptr->idunion.mcarig.dim2;
          fwrite(&bytes,sizeof(int),1,dump);
          fwrite(dptr->idunion.mcarig.f2,dptr->idunion.mcarig.dim2*sizeof(_REAL),
                 1,dump);
          fwrite(&bytes,sizeof(int),1,dump);
          fwrite(&bytes,sizeof(int),1,dump);
          fwrite(dptr->idunion.mcarig.s2,dptr->idunion.mcarig.dim2*sizeof(_REAL),
                 1,dump);
          fwrite(&bytes,sizeof(int),1,dump);
	 }
         fflush(dump);
         fclose(dump);
         printf("S/F matrix elements written as binary fortran.\n");
	 /* */
         printf("Now code stops before relaxation modes.\n");
	 return;
        }
       }
       else
       {

        nn = 3*dptr->idunion.mcarig.nn;
        F = matrix(1,nn,1,nn);
        S = matrix(1,nn,1,nn);

        for (i=1; i<=n1; i++) {
         for (j=i; j<=n1; j++) {
          sum1=0.;
          sum2=0.;
          for (m1=0; m1<3; m1++) 
          {
           m2_0=m1*idelta(i,j);
           for (m2=m2_0; m2<3; m2++) 
           {
            dptr->idunion.mcarig.st1->f[m1][m2] *= fact;
            dptr->idunion.mcarig.ft1->f[m1][m2] *= fact;
            i3=3*(i-1)+m1+1;
            j3=3*(j-1)+m2+1;
/*
            printf("i,j,m1,m2,i3,j3: %d %d %d %d %d %d\n",
             i,j,m1,m2,i3,j3); 
            fflush(stdout);
*/
            S[i3][j3]=dptr->idunion.mcarig.st1->f[m1][m2];
            S[j3][i3]=S[i3][j3];
            F[i3][j3]=dptr->idunion.mcarig.ft1->f[m1][m2];
            F[j3][i3]=F[i3][j3];
           }
           sum1+=dptr->idunion.mcarig.st1->f[m1][m1];
           sum2+=dptr->idunion.mcarig.ft1->f[m1][m1];
          }
/*
          S[i][j]=sum1/3.;
          S[j][i]=S[i][j];
          F[i][j]=sum2/3.;
          F[j][i]=F[i][j];
*/
          dptr->idunion.mcarig.st1++;
          dptr->idunion.mcarig.ft1++;
         }
        }
        dptr->idunion.mcarig.st1 -= dptr->idunion.mcarig.dim1;
        dptr->idunion.mcarig.ft1 -= dptr->idunion.mcarig.dim1;

        if(dptr->idunion.mcarig.sel_bonds!=0)
        {

        for (i=1; i<=n1; i++) {
         for (j=1; j<=n2; j++) {
	  sum1=0.;
          sum2=0.;
          for (m1=0; m1<3; m1++) 
          {
           for (m2=0; m2<3; m2++) 
           {
            dptr->idunion.mcarig.st12->f[m1][m2] *= fact;
            dptr->idunion.mcarig.ft12->f[m1][m2] *= fact;

            i3=3*(i-1)+m1+1;
            j3=3*n1+3*(j-1)+m2+1;
/*
            printf("i,j,m1,m2,i3,j3: %d %d %d %d %d %d ",
             i,j,m1,m2,i3,j3); 
            printf("%e ",
             dptr->idunion.mcarig.st12->f[m1][m2]);
            printf("%e\n",
             dptr->idunion.mcarig.ft12->f[m1][m2]);
            fflush(stdout);
*/
            S[i3][j3]=dptr->idunion.mcarig.st12->f[m1][m2];
            S[j3][i3]=S[i3][j3];
            F[i3][j3]=dptr->idunion.mcarig.ft12->f[m1][m2];
            F[j3][i3]=F[i3][j3];
           }
           sum1+=dptr->idunion.mcarig.st12->f[m1][m1];
           sum2+=dptr->idunion.mcarig.ft12->f[m1][m1];
          }
/*
          S[i][j]=sum1/3.;
          S[j][i]=S[i][j];
          F[i][j]=sum2/3.;
          F[j][i]=F[i][j];
*/
          dptr->idunion.mcarig.st12++;
          dptr->idunion.mcarig.ft12++;
         }
        }
        dptr->idunion.mcarig.st12 -= dptr->idunion.mcarig.dim12;
        dptr->idunion.mcarig.ft12 -= dptr->idunion.mcarig.dim12;
        for (i=1; i<=n2; i++) {
         for (j=i; j<=n2; j++) {

          sum1=0.;
          sum2=0.;
          for (m1=0; m1<3; m1++) 
          {
           m2_0=m1*idelta(i,j);
           for (m2=m2_0; m2<3; m2++) 
           // for (m2=0; m2<3; m2++) 
           {
            dptr->idunion.mcarig.st2->f[m1][m2] *= fact;
            dptr->idunion.mcarig.ft2->f[m1][m2] *= fact;
            i3=3*n1+3*(i-1)+m1+1;
            j3=3*n1+3*(j-1)+m2+1;
/*
            printf("i,j,m1,m2,i3,j3: %d %d %d %d %d %d\n",
             i,j,m1,m2,i3,j3); 
            fflush(stdout);
*/
            S[i3][j3]=dptr->idunion.mcarig.st2->f[m1][m2];
            S[j3][i3]=S[i3][j3];
            F[i3][j3]=dptr->idunion.mcarig.ft2->f[m1][m2];
            F[j3][i3]=F[i3][j3];
           }
           sum1+=dptr->idunion.mcarig.st2->f[m1][m1];
           sum2+=dptr->idunion.mcarig.ft2->f[m1][m1];
          }
/*
          S[i][j]=sum1/3.;
          S[j][i]=S[i][j];
          F[i][j]=sum2/3.;
          F[j][i]=F[i][j];
*/
          dptr->idunion.mcarig.st2++;
          dptr->idunion.mcarig.ft2++;
         }
        }
        dptr->idunion.mcarig.st2 -= dptr->idunion.mcarig.dim2;
        dptr->idunion.mcarig.ft2 -= dptr->idunion.mcarig.dim2;

        }

       }

}

void setup_switch()
{
 int i;
 int iRc,iRl;
 _REAL S,aa,bb,R,R2;
 extern _REAL S_switch[1000];

 iRc=24;
 iRl=12;
 aa=1./(_REAL)(iRc-iRl);
 bb=(_REAL)(iRl)/(_REAL)(iRc-iRl);

 for(i=0;i<1000;i++)
 {
/*
  if(i>=iRc)
  {
   S=0.;
  } 
  else if (i<=iRl)
  {
   S=1.;
  }
  else
  {
   R=aa*i-bb;
   R2=R*R;
   S=R2*(2.*R-3)+1.;
  }
*/
  R=(_REAL)i/(_REAL)iRl;
  R2=R*R;
  S=exp(-R2*0.5);
  /* printf("i,S= %d %e\n",i,S); */
  S_switch[i]=S;
 }

}
